...
run_remote_script() {
    ...
    if curl -fsSL "$url" -o "$tmp"; then
        ...
        bash "$tmp"
        rm -f "$tmp"
        print_success "Script $name executed"
    else
...
show_menu() {
    clear
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}               🚀 FUTURENODE MANAGER           ${NC}"
    ...
    echo -e ""
    read -p "$(echo -e "${YELLOW}Select an option [0-9]: ${NC}")" choice
}
...
while true; do
    ...
    case $choice in
        ...
        9) system_info ;;
        0) 
            echo -e "${GREEN}Exiting FutureNode Manager...${NC}"
